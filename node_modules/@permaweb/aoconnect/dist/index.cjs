...
function relayerWith({ fetch: fetch2, logger, HB_URL: HB_URL2, signer }) {
  return async (url, options) => {
    const parsed = new URL(url);
    options.headers = new Headers(options.headers);
    const relayUrl = parsed.href;
    const relayParams = new URLSearchParams({ "relay-path": relayUrl });
    const hb = `${HB_URL2}/~relay@1.0/call?${relayParams.toString()}`;
...
var DEFAULT_HB_URL = "http://localhost:8734";
...
function connectWith({ createDataItemSigner: createDataItemSigner3, createHbSigner: createHbSigner2 }) {
  ...
  function connect2({
     ...
     */
    originalFetch = fetch2
  } = {}) {
    if (!GRAPHQL_URL2) GRAPHQL_URL2 = joinUrl({ url: GATEWAY_URL2, path: "/graphql" });
    const { validate } = (0, import_ao_scheduler_utils.connect)({ cacheSize: 100, GRAPHQL_URL: GRAPHQL_URL2, GRAPHQL_MAX_RETRIES: GRAPHQL_MAX_RETRIES2, GRAPHQL_RETRY_BACKOFF: GRAPHQL_RETRY_BACKOFF2 });
    ...
    return { result: result2, results: results2, message: message2, spawn: spawn2, monitor: monitor2, unmonitor: unmonitor2, dryrun: dryrun2, assign: assign2, createDataItemSigner: createDataItemSigner3 };
  }
  connect2.hb = function({
    wallet,
    ...
    const api = connect2({
      ...
      fetch: relayerWith({
        ...
        logger,
        HB_URL: URL2,
        signer: createHbSigner2(wallet)
      })
    });
    const createDataItemSigner4 = api.createDataItemSigner;
    api.createDataItemSigner = () => createDataItemSigner4(wallet);
    return api;
  };
  return connect2;
}
...
var { signMessage } = import_http_message_signatures.httpbis;
...
function createHbSigner(wallet) {
  ...
  const address = (0, import_node_crypto.createHash)("sha256").update(Buffer.from(publicKey, "base64url")).digest("base64url");
  const s = (0, import_http_message_signatures.createSigner)(privateKey, "rsa-pss-sha512", publicKey);
...
var HB_URL = process.env.HB_URL || void 0;
...
var originalHb = connect.hb;
connect.hb = ({ URL: URL2 = HB_URL, ...rest }) => originalHb({ URL: URL2, ...rest });
